...
BASE_DIR = os.path.expanduser("~/climate_project/data")
...
print("--- 1. MERGE DES FICHIERS BRUTS ---")
raw_files = [
    os.path.join(RAW_DIR, "SIM_mensuelle_2000_2009.csv"),
    os.path.join(RAW_DIR, "SIM_mensuelle_2010_2019.csv"),
    os.path.join(RAW_DIR, "SIM_mensuelle_2020_2026.csv")
]
...
for file in raw_files:
    if os.path.exists(file):
        ...
        try:
            # Essai de lecture directe en mode décompression automatique
            ...
            try:
                ...
                # Si le fichier est un CSV texte normal non compressé
...
# Sauvegarde brute dans interim
interim_path = os.path.join(INTERIM_DIR, "SIM_mensuelle_2000_2026_merged.csv")
df_merged.to_csv(interim_path, index=False)
print(f"✅ Fichier fusionné sauvegardé dans : {interim_path}")

print("\n--- 2. NETTOYAGE ET FILTRAGE (Arrêt à Décembre 2025) ---")
...
if date_cols:
    ...
    df_merged['DATE_DT'] = pd.to_datetime(df_merged[date_col].astype(str), format='%Y%m', errors='coerce')
...
# Sauvegarde finale dans processed
processed_path = os.path.join(PROCESSED_DIR, "SIM_mensuelle_2000_2025_cleaned.csv")
df_clean.to_csv(processed_path, index=False)
print(f"\n🎉 Terminé ! Dataset propre généré : {processed_path}")
